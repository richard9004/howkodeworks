...
function CreateSubCategory(props) {
  const { isOpen, onClose, refreshCategories, allCats } = props;
  ...
  const postCategoryData = async (data) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/addSubCategory`,
        {
          title: catData.title,
      ...
      // REALTIME EXAMPLES
      toast.success("Success ! Sub Category has been created", {
  ...
  return (
    <Modal
      ...
      style={customStyles}
      contentLabel="Create Sub New Category"
    >
      <form
      ...
      >
        <div className="modal-header">
          <h5 className="modal-title">Create New Sub Category</h5>
          <button
            type="button"
            className="btn-close"
            onClick={closeModal}
  ...
  >
    ...
    {allCats && allCats.data && allCats.data.length > 0 && allCats.data.map(category => (
