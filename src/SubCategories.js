...
function SubCategories() {
  ...
  const static_meta_title = "Manage SubCategories - LetsMakeItOnline";
  const static_meta_description= "Add, Update and Delete SubCategories Here";
  ...
  useEffect(() => {
    ...
    getCategories();
    updateStaticMeta();
  }, [searchTitle, searchCategory, searchSubCategory, searchStatus, searchCategoryTitle]);
  ...
  const getCategories = async (pageNo = 1) => {
    ...
    const search = {
      title: searchTitle,
      category_title:searchCategoryTitle
    };

    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/getSubCategories?page=${pageNo}`,
        {
          headers,
          params: search,
        }
...
setIsViewModalOpen(true);
  ...
  <div className="card-body d-flex justify-content-between align-items-center">
    <div>
      <h3 className="card-title"> Manage Sub Categories <i className="fa fa-file-alt"></i></h3>
    ...
    <button className="btn btn-custom"  onClick={() => openCreateCategoryModal()}><i className="fa fa-plus"></i> Add Sub Category</button>
  </div>
</div>
  ...
  <tr className="text-center">
    <td colSpan="3">
      ...
      <CreateSubCategory isOpen={isCreateCategoryOpen} onClose={onCloseCreateCategory} refreshCategories={refreshCategories} allCats={allCats}/>
     ...
     {Object.keys(tempCatData).length > 0 && (
       <EditSubCategory isOpen={isEditModalOpen} onClose={onCloseEditModal} refreshCategories={refreshCategories} tempCatData={tempCatData}  allCats={allCats}/>
